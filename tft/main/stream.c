...
static void wifi_sta_init(void)
{
    ...
    ESP_ERROR_CHECK(esp_netif_str_to_ip4(TFT_STATIC_IP, &ip_info.ip));
    ESP_ERROR_CHECK(esp_netif_str_to_ip4(TFT_GATEWAY, &ip_info.gw));
    ESP_ERROR_CHECK(esp_netif_str_to_ip4(TFT_NETMASK, &ip_info.netmask));
    ESP_ERROR_CHECK(esp_netif_set_ip_info(sta_netif, &ip_info));
    ESP_LOGI(TAG, "Statische IP gesetzt: %s (GW %s)", TFT_STATIC_IP, TFT_GATEWAY);
...
static void fetch_task(void *arg)
{
    ...
    char *h = nvs_config_get_str("tft_host", "");
